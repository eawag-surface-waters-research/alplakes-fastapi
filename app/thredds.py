...
def get_meteodata_station_metadata(filesystem, station_id):
    variables_convert = {
        ...
        "glo": "global_radiation",
        "ta": "air_temperature",
        "cumul_precip": "precipitation",
        "ws": "wind_speed",
        "wd": "wind_direction"}
    variables_dict = functions.meteostation_variables()
    out = {"id": station_id}
    station_dir = os.path.join(filesystem, "media/thredds/meteodata", station_id)
    stations_file = os.path.join(filesystem, "media/thredds/meteodata/stations.json")
...
def get_meteodata_measured(filesystem, station_id, variables, start_date, end_date):
    ...
    variables_adjust = {
        ...
        "global_radiation": functions.hourly_joules_to_watts
    }
    variables_dict = functions.meteostation_variables()
    station_dir = os.path.join(filesystem, "media/thredds/meteodata", station_id)
